...
def load_kg(data_dir, graph_file_name):
    print("loading {}....".format(graph_file_name))
    data_path = os.path.join(data_dir, graph_file_name)
...
def split_dataset(txt_path, output_dir):
    lines = open(txt_path).readlines()
    random.shuffle(lines)
    total_num = len(lines)
    # split ratio train : dev : test = 0.9 : 0.05 : 0.05
    train_num = round(total_num * 0.9)
    dev_num = round(total_num * 0.05)

    train_dataset_path = os.path.join(output_dir, "train.txt")
    dev_dataset_path = os.path.join(output_dir, "dev.txt")
    test_dataset_path = os.path.join(output_dir, "test.txt")

    open(train_dataset_path, 'w').writelines(lines[:train_num])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_file_name', type = str, default ="conceptnet_graph", help='graph file name')
    parser.add_argument('--split_dataset', type = bool, default =True, help='if you want to split into train/dev/test, true')
    ...
    print('loading kg')
    kg_full, kg_simple = load_kg(data_dir, graph_file_name)
    ...
    if args.split_dataset:
        split_dataset(output_path, output_dir)
